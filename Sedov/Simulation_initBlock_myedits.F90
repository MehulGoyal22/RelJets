...
#include "Flash.h"
  ...
  ! Local jet parameters (hardcoded since we can't modify Simulation_data)
  real, parameter :: jet_theta = 10.0          ! Jet half-opening angle in degrees
  real, parameter :: jet_axis_x = 1.0          ! Jet direction (x-component)
  real, parameter :: jet_axis_y = 0.0          ! Jet direction (y-component)
  real, parameter :: jet_axis_z = 0.0          ! Jet direction (z-component)
! logical, parameter :: sim_useJet = .true.    ! Enable jet mode 
  real, parameter :: pi = 3.141592653589793    ! Pi constant
     ...
     do j = blkLimitsGC(LOW, JAXIS), blkLimitsGC(HIGH, JAXIS)
        ...
        do i = blkLimitsGC(LOW,IAXIS), blkLimitsGC(HIGH, IAXIS)
           ...
           ! 
            ...
            real :: theta_jet_rad, axis_norm
            real :: nx, ny, nz
            real :: theta, r_cyl

            theta_jet_rad = (10) * (pi/180.0)
            axis_norm = sqrt(jet_axis_x**2 + jet_axis_y**2 + jet_axis_z**2)
            nx = jet_axis_x / axis_norm
            ny = jet_axis_y / axis_norm
            nz = jet_axis_z / axis_norm
           ...
           do kk = 0, (sim_nSubZones-1)*K3D
              ...
              do jj = 0, (sim_nSubZones-1)*K2D
                 ...
                 do ii = 0, (sim_nSubZones-1)
                    ...
                    r_cyl   = sqrt(yDist**2 + xDist**2)  ! Cylindrical radius
                    theta   = atan2(r_cyl, xDist)        ! Angle from x-axis (jet axis)

                if (theta <= theta_jet_rad) then
